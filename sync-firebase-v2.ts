...
// Coba fetch dari multiple server endpoints
async function tryFetchData(endpoint: string): Promise<any[]> {
  for (const port of SERVER_PORTS) {
    ...
    try {
      ...
      console.log(`    ✅ Berhasil dari ${serverBase}`);
      return data;
    } catch (e) {
      // Continue to next server
      console.log(`    ❌ Gagal port ${port}`);
    }
  }
  return [];
}
...
async function syncToFirebase() {
  ...
  try {
    ...
    // 2. Fetch data dari server lokal dengan retry multiple ports
    ...
    try {
      ...
      attendanceData = await tryFetchData('attendance');
      console.log(`    ✓ Kehadiran: ${attendanceData.length} records\n`);
    } catch (e) {
      console.log('  ⚠ Tidak bisa fetch attendance\n');
    }

    try {
      console.log('  👶 Fetch Students:');
      studentsData = await tryFetchData('students');
      console.log(`    ✓ Siswa: ${studentsData.length} records\n`);
    } catch (e) {
      console.log('  ⚠ Tidak bisa fetch students\n');
    }

    try {
      console.log('  👨‍🏫 Fetch Teachers:');
      teachersData = await tryFetchData('teachers');
      console.log(`    ✓ Guru: ${teachersData.length} records\n`);
    } catch (e) {
      console.log('  ⚠ Tidak bisa fetch teachers\n');
    }

    // 3. Filter data yang tidak dihapus
    console.log('🔍 Memfilter data aktif...');
    ...
    console.log('📤 Mengirim data ke Firebase Firestore...\n');
