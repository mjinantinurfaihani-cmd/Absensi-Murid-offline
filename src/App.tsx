...
import{debugTeacherSync,debugManualPublish}from'./debugPersistence';
import BackupControls from './components/BackupControls';

// Expose debug tools to browser console
if(typeof window!=='undefined'){(window as any).debugTeacherSync=debugTeacherSync;(window as any).debugManualPublish=debugManualPublish;}
...
export default function App(){const[user,setUser]=useState<UserSession|null>(()=>{try{return normalizeUserSession(JSON.parse(localStorage.getItem('session')||'null'));}catch{return null;}});const[ready,setReady]=useState(false);const[conflicts,setConflicts]=useState<AttendanceConflict[]>([]);const[feedback,setFeedback]=useState('');const[fading,setFading]=useState<string|null>(null);const{toast,showToast,closeToast}=useToast();const[conflictDebugEnabled,setConflictDebugEnabled]=useState(()=>localStorage.getItem('conflictDebug')==='1');
...
function Scan({user,showToast}:any){
  ...
  async function load(){
    ...
    setClassOptions([...new Set(students.filter(student=>!student.deleted).map(student=>student.kelas.trim()).filter(Boolean))].sort((a,b)=>compareText(a,b)));
