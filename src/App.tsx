function Students({user,showToast}:any){return <><StudentsTable user={user} showToast={showToast}/>{user.role==='admin'&&<div className="heading-actions"><button className="danger" onClick={()=>void deleteAllStudents(showToast)}>Hapus Semua Data Siswa</button></div>}</>}
...
function Scan({user,showToast}:any){
  ...
  return <>
    <section className="toolbar card">
      ...
      <button onClick={()=>syncAll().then(result=>showToast('success',`${result.sent} data tersinkron`)).catch((error:unknown)=>showToast('error',error instanceof Error?error.message:'Sinkronisasi gagal'))}>Sinkronkan</button>
    </section>
...
function ExcelActions({kind,showToast,after}:{kind:'siswa'|'guru';showToast:any;after:()=>void}){const input=useRef<HTMLInputElement>(null);async function change(e:any){const file=e.target.files?.[0];if(!file)return;try{const result=kind==='siswa'?await importStudents(file):await importTeachers(file);showToast('success',`Import selesai: ${result.added} baru, ${result.updated} diperbarui, ${result.skipped} dilewati`);after()}catch(err:any){showToast('error',err.message||'Import Excel gagal')}finally{e.target.value=''}}return <div className="excel-actions"><input ref={input} type="file" accept=".xlsx,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" hidden onChange={change}/><button onClick={()=>downloadTemplate(kind)}>Template</button><button onClick={()=>input.current?.click()}>Import .xlsx</button><button className="primary" onClick={()=>{const action=kind==='siswa'?exportStudents():exportTeachers();action.then(n=>showToast('success',`${n} data berhasil diekspor`)).catch((e:any)=>showToast('error',e.message))}}>Export .xlsx</button></div>}
