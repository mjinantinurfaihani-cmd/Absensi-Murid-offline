function StudentsPage({user,showToast}:any){const[refreshKey,setRefreshKey]=useState(0);useEffect(()=>{const refresh=()=>setRefreshKey(value=>value+1);window.addEventListener('public-data-updated',refresh);return()=>window.removeEventListener('public-data-updated',refresh)},[]);return <><StudentList key={refreshKey} user={user} showToast={showToast}/>{user.role==='admin'&&<div className="heading-actions"><button className="danger" onClick={async()=>{await deleteAllStudents(showToast);setRefreshKey(value=>value+1)}}>Hapus Semua Data Siswa</button></div>}</>}
...
async function deleteAllTeachers(showToast:any){if(!window.confirm('Apakah anda yakin ingin menghapus semua data guru? Akun admin tetap dipertahankan.'))return;const teachers=await db.teachers.filter(teacher=>!teacher.deleted&&teacher.role!=='admin').toArray();if(!teachers.length){showToast('info','Tidak ada data guru non-admin untuk dihapus');return}await db.teachers.bulkDelete(teachers.map(teacher=>teacher.id));showToast('success','Semua data guru non-admin berhasil dihapus')}
import{useEffect,useMemo,useRef,useState}from'react';import{BrowserMultiFormatReader}from'@zxing/browser';import*as XLSX from'xlsx';import{saveAs}from'file-saver';import{db,seed}from'./db';import type{Attendance,AttendanceConflict,SoundMode,Student,Teacher,UserSession}from'./types';import { getStudentsFromSql, getTeachersFromSql, sqlQuery, syncSqlToIndexedDB } from './sqlStore';import{AnimatedToast,PresetToolbar,useToast,type PresetId}from'./toast';import{notify}from'./notify';import{confirmAttendanceConflict,syncAll}from'./sync';import{syncAllDestinations}from'./enhancedSync';import{downloadTemplate,exportStudents,exportTeachers,importStudents,importTeachers}from'./excel';import QrCardsPanel from './components/QrCardsPanel';
...
function AttendanceChart(){const user=JSON.parse(localStorage.getItem('session')||'null') as UserSession;const[students,setStudents]=useState<Student[]>([]);const[attendance,setAttendance]=useState<Attendance[]>([]);const[query,setQuery]=useState('');const[kelas,setKelas]=useState('ALL');useEffect(()=>{Promise.all([db.students.filter(s=>!s.deleted).toArray(),db.attendance.toArray()]).then(([s,a])=>{setStudents(s.sort((x,y)=>x.nama.localeCompare(y.nama)));setAttendance(a)})},[]);const start=new Date();start.setHours(0,0,0,0);start.setMonth(start.getMonth()-5,1);const startKey=start.toLocaleDateString('en-CA');const endKey=today();const assignedClasses=user.role==='admin'?null:user.kelas.split(',').map((value:string)=>value.trim()).filter(Boolean);const classes=[...new Set(students.map(s=>s.kelas))].sort();const data=students.filter(s=>(!assignedClasses||assignedClasses.includes(s.kelas))&&(kelas==='ALL'||s.kelas===kelas)&&(!query||`${s.nama} ${s.nisn}`.toLowerCase().includes(query.toLowerCase()))).map(student=>{const records=attendance.filter(row=>row.studentId===student.id&&row.tanggal>=startKey&&row.tanggal<=endKey);return{student,values:[records.filter(r=>r.status==='HADIR'||r.status==='TERLAMBAT').length,records.filter(r=>r.status==='SAKIT').length,records.filter(r=>r.status==='IZIN').length,records.filter(r=>r.status==='ALPA').length]}});const max=Math.max(1,...data.map(item=>Math.max(...item.values)));function download(){const svg=document.getElementById('attendance-chart-svg');if(!svg)return;const blob=new Blob([new XMLSerializer().serializeToString(svg)],{type:'image/svg+xml'});const link=document.createElement('a');link.href=URL.createObjectURL(blob);link.download='Grafik_Kehadiran_6_Bulan.svg';link.click();URL.revokeObjectURL(link.href)}return <><div className="heading"><div><h1>Grafik Kehadiran</h1><p className="muted">Rekap per siswa selama enam bulan terakhir.</p></div><button className="primary" onClick={download} disabled={!data.length}>Download grafik</button></div><section className="card list-tools"><input placeholder="Cari nama atau NISN" value={query} onChange={e=>setQuery(e.target.value)}/><select value={kelas} onChange={e=>setKelas(e.target.value)}><option value="ALL">Semua kelas</option>{classes.map(value=><option key={value}>{value}</option>)}</select></section><section className="card chart-scroll"><div className="chart-legend"><span><i className="legend-present"/>Hadir</span><span><i className="legend-sick"/>Sakit</span><span><i className="legend-permit"/>Ijin</span><span><i className="legend-absent"/>Alpa</span></div><svg id="attendance-chart-svg" className="attendance-chart" viewBox={`0 0 ${Math.max(760,data.length*92)} 460`} role="img" aria-label="Grafik batang kehadiran enam bulan"><line x1="50" y1="390" x2={Math.max(720,data.length*92)} y2="390" stroke="#9bcfe3"/>{data.map((item,index)=>{const x=70+index*92;return <g key={item.student.id}><text x={x+25} y="420" textAnchor="middle" className="chart-label">No. {index+1}</text>{item.values.map((value,statusIndex)=><rect key={statusIndex} x={x+statusIndex*16} y={390-(value/max*330)} width="12" height={value/max*330} rx="4" className={`bar-${statusIndex}`}><title>{attendanceStatusLabel(statusIndex===0?'HADIR':statusIndex===1?'SAKIT':statusIndex===2?'IZIN':'ALPA')}: {value}</title></rect>)}</g>})}</svg>{!data.length&&<p className="empty">Data grafik tidak ditemukan.</p>}</section></>}
...
function Scan({user,showToast}:any){
  ...
  return <>
    <section className="toolbar card">
      ...
      <button onClick={()=>syncAllDestinations().then(result=>showToast(result.status==='success'?'success':'warning',result.message)).catch((error:unknown)=>showToast('error',error instanceof Error?error.message:'Sinkronisasi gagal'))}>Sinkronkan (Server + Firebase + Git)</button>
    </section>
...
function StudentList({user,showToast}:any){const[list,setList]=useState<Student[]>([]);const[query,setQuery]=useState('');const[kelas,setKelas]=useState('ALL');const[page,setPage]=useState(1);const[edit,setEdit]=useState<Partial<Student>>({kelas:user.role==='guru'?user.kelas:'4A'});const[open,setOpen]=useState(false);const pageSize=10;const normalizeClass=(value:string)=>value.trim().toUpperCase();const sessionClasses=user.role==='admin'?null:user.kelas.split(',').map((value:string)=>normalizeClass(value)).filter(Boolean);const[assignedClasses,setAssignedClasses]=useState<string[]|null>(sessionClasses);const classes=[...new Set([...list.map(s=>s.kelas),edit.kelas||'4A'])].filter(value=>!assignedClasses||assignedClasses.includes(normalizeClass(value))).sort();async function load(){
    try{
      let a = await db.students.filter(s=>!s.deleted).toArray();
      const teacher=user.role==='admin'?null:await db.teachers.get(user.id);
      const currentClasses=teacher&&!teacher.deleted?teacher.kelas.split(',').map((value:string)=>normalizeClass(value)).filter(Boolean):sessionClasses||[];
      if(user.role!=='admin')setAssignedClasses(currentClasses);
      if(user.role!=='admin')a = a.filter(s=>currentClasses.includes(normalizeClass(s.kelas)));
      ...
      let a = await db.students.filter(s=>!s.deleted).toArray();
      if(assignedClasses) a = a.filter(s=>assignedClasses.includes(normalizeClass(s.kelas)));
      setList(a.sort((a,b)=>a.nama.localeCompare(b.nama)));
    }
  }
...
INFOBIP_TEMPLATE_LANGUAGE=id</pre></div><p className="muted">Saat status non-aktif, absensi tetap berjalan normal dan notifikasi WhatsApp akan otomatis dilewati.</p><section className="card form"><h2>Sinkronisasi GitHub</h2><p className="muted">Sinkronkan semua data absensi ke repository GitHub Anda. Diperlukan GitHub Personal Access Token dengan akses ke repository.</p><label>GitHub Personal Access Token<input type="password" value={githubToken} onChange={e=>setGithubToken(e.target.value)} placeholder="ghp_xxxxxxxxxxxxx" title="Token dengan scope: repo, workflow"/></label><label>GitHub Username<input type="text" value={githubUsername} onChange={e=>setGithubUsername(e.target.value)} placeholder="mjinantinurfaihani-cmd"/></label><label>Repository Name<input type="text" value={githubRepo} onChange={e=>setGithubRepo(e.target.value)} placeholder="Absensi-Murid-offline"/></label><div className="button-group"><button disabled={testingGithub} onClick={()=>void saveGithubConfig()}>{testingGithub?'Menyimpan...':'Simpan Konfigurasi GitHub'}</button>{githubToken&&<button className="danger" onClick={clearGithubConfig}>Hapus Konfigurasi</button>}</div><p className="muted"><strong>Cara mendapat token:</strong><br/>1. Buka <a href="https://github.com/settings/tokens" target="_blank">https://github.com/settings/tokens</a><br/>2. Klik "Generate new token"<br/>3. Pilih scope: <code>repo, workflow</code><br/>4. Copy token dan paste di sini</p></section><section className="card form"><h2>Server Sinkronisasi Opsional</h2><label>URL server<input defaultValue={localStorage.getItem('serverUrl')||''} placeholder="https://server-anda/api" onBlur={e=>localStorage.setItem('serverUrl',e.target.value.trim())}/></label><button onClick={()=>showToast('info','URL server tersimpan')}>Simpan</button></section></>}