...
async function deleteAllStudents(showToast:any){if(!window.confirm('Apakah anda yakin ingin menghapus semua data siswa secara permanen?'))return;const students=await db.students.toArray();if(!students.length){showToast('info','Tidak ada data siswa untuk dihapus');return;}await db.transaction('rw', db.students, db.attendance, async () => {for (const student of students) { await db.attendance.where('studentId').equals(student.id).delete(); } await db.students.bulkDelete(students.map(student => student.id)); });await Promise.all(students.map(student => Promise.allSettled([removePublicStudent(student.id), removePublicStudentAttendanceTrace(student.id)])));showToast('success','Semua data siswa berhasil dihapus dari lokal dan cloud')}
import{useEffect,useMemo,useRef,useState,useImperativeHandle,forwardRef}from'react';import{BrowserMultiFormatReader}from'@zxing/browser';import*as XLSX from'xlsx';import{saveAs}from'file-saver';import{db,seed,hardDeleteStudent,hardDeleteTeacher}from'./db';import type{Attendance,AttendanceConflict,SoundMode,Student,Teacher,UserSession}from'./types';import { getStudentsFromSql, getTeachersFromSql, sqlQuery, syncSqlToIndexedDB } from './sqlStore';import{AnimatedToast,PresetToolbar,useToast,type PresetId}from'./toast';import{notify,playGlitchSound}from'./notify';import{confirmAttendanceConflict,syncAdminDirectoryData,syncAll}from'./sync';import{downloadTemplate,exportAttendanceSnapshot,exportStudents,exportTeachers,importStudents,importTeachers}from'./excel';import QrCardsPanel from './components/QrCardsPanel';
...
function normalizeUserSession(raw:any): UserSession | null {
  ...
  const candidateId=typeof raw.id==='string'&&raw.id?raw.id:(typeof raw.teacherId==='string'&&raw.teacherId?raw.teacherId:'');
  const role=raw.role==='admin'||raw.role==='guru'||raw.role==='guru bidang'?raw.role:null;
  if(!candidateId||!role)return null;
  const kelas=typeof raw.kelas==='string'?raw.kelas:(typeof raw.teacherClasses==='string'?raw.teacherClasses:'');
  return {id:candidateId,nama:typeof raw.nama==='string'?raw.nama:'',role,kelas};
}
...
const compareTime=(left:unknown,right:unknown)=>safeText(left).localeCompare(safeText(right), 'en', {numeric: true, sensitivity: 'base'});
...
export default function App() {
  ...
  useEffect(()=>{seed().then(()=>{setReady(true);loadStudentsMap();});const online=async()=>{const serverUrl=(localStorage.getItem('serverUrl')||'').trim();if(!serverUrl||!navigator.onLine){setSyncStatus('offline');return;}setSyncStatus('syncing');try{const result=await syncAll();if(user?.role==='admin'){await syncAdminDirectoryData();}
        const unresolved=result.conflicts.filter(conflict=>!conflict.resolved); // update state
...
function Scan({user,showToast}:any){
  if(!user) return <div style={{padding:'2rem',textAlign:'center',color:'#999'}}>Memuat data pengguna...</div>;
  const userClasses=getAssignedClasses(user);
  const[rows,setRows]=useState<ScanHistoryRow[]>([]);
  const[classOptions,setClassOptions]=useState<string[]>([]);
  const[kelas,setKelas]=useState<string>(user.role==='guru'?(userClasses[0]||'ALL'):'ALL');
  ...
  async function load(){
    ...
    setClassOptions([...new Set(students.filter(student=>!student.deleted).flatMap(student=>normalizeClassList(student.kelas)))].sort((a,b)=>compareText(a,b)));
    setTeacherOptions(teachers.sort((a,b)=>compareText(a.nama,b.nama)).map(teacher=>({id:teacher.id,nama:teacher.nama,role:teacher.role,kelas:teacher.kelas || ''})));
    ...
    const scoped=history.filter(row=>(user.role==='guru bidang'?(userClasses.includes(row.kelas)&&(kelas==='ALL'||row.kelas===kelas)):(kelas==='ALL'||row.kelas===kelas))&&(!query||row.nama.toLowerCase().includes(query)||row.nisn.toLowerCase().includes(query))&& (teacherFilter==='ALL'||row.ownerId===teacherFilter || (user.role !== 'admin' && row.ownerId===user.id)));
    if(user.role==='guru bidang'){
      ...
      setRows([...grouped.values()].map(group=>{const own=group.find(row=>ownsAttendance(row,user));const classRecord=group.find(row=>!ownsAttendance(row,user)&&row.ownerRole==='guru');return {...(own||classRecord||group[0]),comparisonStatus:own&&classRecord&&own.status!==classRecord.status?classRecord.status:undefined}}).sort((a,b)=>compareTime(b.jamMasuk,a.jamMasuk)));
    }else setRows((user.role==='admin'?scoped:scoped.filter(row=>ownsAttendance(row,user))).sort((a,b)=>compareTime(b.jamMasuk,a.jamMasuk)));
      ...
      <div className="filters" style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(180px,1fr))',gap:'.75rem',alignItems:'end'}}>
        ...
        <label style={{display:'grid',gap:'.35rem',fontSize:'.8rem',fontWeight:700,color:'#1e3a5f'}}>
          ...
          <select value={kelas} onChange={event=>setKelas(event.target.value)}>
            ...
            {(user.role==='guru bidang'?userClasses:classOptions).map((value:string)=><option key={value} value={value}>{value}</option>)}
          </select>
        </label>
        <label style={{display:'grid',gap:'.35rem',fontSize:'.8rem',fontWeight:700,color:'#1e3a5f'}}>
          <span>Guru</span>
...
function StudentList({user,showToast}:any){if(!user) return <div style={{padding:'2rem',textAlign:'center',color:'#999'}}>Memuat data siswa...</div>;const[list,setList]=useState<Student[]>([]);const[query,setQuery]=useState('');const[kelas,setKelas]=useState('ALL');const[page,setPage]=useState(1);const[edit,setEdit]=useState<Partial<Student>>({kelas:user.role==='guru'?getPrimaryClass(user):'4A'});const[open,setOpen]=useState(false);const pageSize=10;const assignedClasses=user.role==='admin'?null:getAssignedClasses(user);const classes=[...new Set([...list.map(s=>s.kelas),edit.kelas||'4A'])].filter(value=>!assignedClasses||assignedClasses.includes(value)).sort();async function load(){
    try{
      let a = await db.students.filter(s=>!s.deleted).toArray();
      if(assignedClasses) a = a.filter(s=>assignedClasses.includes(s.kelas));
      setList(a.sort((a,b)=>compareText(a.nama,b.nama)));
    }catch(e){
